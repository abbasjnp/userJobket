...
isResizing=!0;var windowsWidth=$window.outerWidth();windowsHeight=$window.height();if(options.resize){resizeMe(windowsHeight,windowsWidth)}
...
isResizing=!1;$.isFunction(options.afterResize)&&resizing&&options.afterResize.call(container);$.isFunction(options.afterReBuild)&&!resizing&&options.afterReBuild.call(container)};FP.setResponsive=function(active){var isResponsive=$body.hasClass(RESPONSIVE);if(active){if(!isResponsive){FP.setAutoScrolling(!1,'internal');FP.setFitToSection(!1,'internal');$(SECTION_NAV_SEL).hide();$body.addClass(RESPONSIVE)}}
...
var slideMoving=!1;var isTouchDevice=navigator.userAgent.match(/(iPhone|iPod|iPad|Android|playbook|silk|BlackBerry|BB10|Windows Phone|Tizen|Bada|webOS|IEMobile|Opera Mini)/);var isTouch=(('ontouchstart' in window)||(navigator.msMaxTouchPoints>0)||(navigator.maxTouchPoints));var container=$(this);var windowsHeight=$window.height();var isResizing=!1;var isWindowFocused=!0;var lastScrolledDestiny;var lastScrolledSlide;var canScroll=!0;var scrollings=[];var nav;var controlPressed;var isScrollAllowed={};isScrollAllowed.m={'up':!0,'down':!0,'left':!0,'right':!0};isScrollAllowed.k=$.extend(!0,{},isScrollAllowed.m);var originals=$.extend(!0,{},options);var resizeId;var afterSectionLoadsId;var afterSlideLoadsId;var scrollId;var scrollId2;var keydownId;if($(this).length){init();bindEvents()}
function init(){if(options.css3){options.css3=support3d()}
options.scrollBar=options.scrollBar||options.hybrid;setOptionsFromDOM();prepareDom();FP.setAllowScrolling(!0);FP.setAutoScrolling(options.autoScrolling,'internal');var activeSlide=$(SECTION_ACTIVE_SEL).find(SLIDE_ACTIVE_SEL);if(activeSlide.length&&($(SECTION_ACTIVE_SEL).index(SECTION_SEL)!==0||($(SECTION_ACTIVE_SEL).index(SECTION_SEL)===0&&activeSlide.index()!==0))){silentLandscapeScroll(activeSlide)}
responsive();setBodyClass();if(document.readyState==='complete'){scrollToAnchor()}
$window.on('load',scrollToAnchor)}
function bindEvents(){$window.on('scroll',scrollHandler).on('hashchange',hashChangeHandler).blur(blurHandler).resize(resizeHandler);$document.keydown(keydownHandler).keyup(keyUpHandler).on('click touchstart',SECTION_NAV_SEL+' a',sectionBulletHandler).on('click touchstart',SLIDES_NAV_LINK_SEL,slideBulletHandler).on('click',SECTION_NAV_TOOLTIP_SEL,tooltipTextHandler);$(SECTION_SEL).on('click touchstart',SLIDES_ARROW_SEL,slideArrowHandler);if(options.normalScrollElements){$document.on('mouseenter',options.normalScrollElements,function(){FP.setMouseWheelScrolling(!1)});$document.on('mouseleave',options.normalScrollElements,function(){FP.setMouseWheelScrolling(!0)})}}
...
if(options.fitToSection){clearTimeout(scrollId2);scrollId2=setTimeout(function(){if(canScroll&&options.fitToSection){if($(SECTION_ACTIVE_SEL).is(currentSection)){isResizing=!0}
scrollPage($(SECTION_ACTIVE_SEL));isResizing=!1}},options.fitToSectionDelay)}}}
...
if(scrollable.length>0){if(options.scrollOverflowHandler.isScrolled(check,scrollable)){scrollSection()}else{return!0}}else{scrollSection()}}
...
stopMedia(v.activeSection);element.addClass(ACTIVE).siblings().removeClass(ACTIVE);lazyLoad(element);canScroll=!1;setState(slideIndex,slideAnchorLink,v.anchorLink,v.sectionIndex);performMovement(v);lastScrolledDestiny=v.anchorLink;activateMenuAndNav(v.anchorLink,v.sectionIndex)}
function performMovement(v){if(options.css3&&options.autoScrolling&&!options.scrollBar){var translate3d='translate3d(0px, -'+v.dtop+'px, 0px)';transformContainer(translate3d,!0);if(options.scrollingSpeed){afterSectionLoadsId=setTimeout(function(){afterSectionLoads(v)},options.scrollingSpeed)}else{afterSectionLoads(v)}}
...
var oldPageY=0;function mouseMoveHandler(e){if(canScroll){if(e.pageY<oldPageY&&isScrollAllowed.m.up){FP.moveSectionUp()}
...
return events}
function silentLandscapeScroll(activeSlide,noCallbacks){FP.setScrollingSpeed(0,'internal');if(typeof noCallbacks!=='undefined'){isResizing=!0}
...
else if(options.css3){var translate3d='translate3d(0px, -'+top+'px, 0px)';transformContainer(translate3d,!1)}
...
if(options.continuousVertical&&options.scrollBar){options.continuousVertical=!1;showError('warn','Option `scrollBar` is mutually exclusive with `continuousVertical`; `continuousVertical` disabled')}
